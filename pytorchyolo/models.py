...
class Darknet(nn.Module):
    ...
    def module_profiling(self, name, module, x):
        if isinstance(module, nn.Sequential):
            children = list(module.named_children())
            for child_name, child in children:
                x = self.module_profiling(f'{name}_{child_name}', child, x)

        # elif module.__class__ in (nn.Conv2d, nn.BatchNorm2d, nn.LeakyReLU, nn.MaxPool2d, nn.ZeroPad2d, nn.Upsample):
        else:
            t0 = time.time()
            x = module(x)
            dt = (time.time() - t0) * 1000  # ms
            self.profile.append([name, dt])

        return x

    def forward(self, x):
        img_size = x.size(2)
        device_str = "gpu" if x.is_cuda else "cpu"
        layer_outputs, yolo_outputs = [], []
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
                x = self.module_profiling(f'{i}_{device_str}', module, x)
            elif module_def["type"] == "route":
                t0 = time.time()
                ...
                dt = (time.time() - t0) * 1000  # ms
                self.profile.append([f'{i}_{device_str}_route', dt])
            elif module_def["type"] == "shortcut":
                t0 = time.time()
                layer_i = int(module_def["from"])
                x = layer_outputs[-1] + layer_outputs[layer_i]
                dt = (time.time() - t0) * 1000  # ms
                self.profile.append([f'{i}_{device_str}_shortcut', dt])
            elif module_def["type"] == "yolo":
                t0 = time.time()
                x = module[0](x, img_size)
                dt = (time.time() - t0) * 1000  # ms
                self.profile.append([f'{i}_{device_str}_yolo', dt])
