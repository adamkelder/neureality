...
def detect_image(model, image, img_size=416, conf_thres=0.5, nms_thres=0.5):
    ...
    :param nms_thres: IOU threshold for non-maximum suppression, defaults to 0.5
    :type nms_thres: float, optional
    :return: profile list, Detections on image with each detection in the format: [x1, y1, x2, y2, confidence, class]
    :rtype: list, nd.array
    """
    profile = []
    ...
    t0 = time.time()
    input_img = DEFAULT_TRANSFORMS((image, np.zeros((1, 5))))[0].unsqueeze(0)
    profile.append(['preproc_to_tensor', (time.time() - t0) * 1000])

    if torch.cuda.is_available():
        t0 = time.time()
        input_img = input_img.to("cuda")
        profile.append(['preproc_to_gpu', (time.time() - t0) * 1000])
    ...
    with torch.no_grad():
        ...
        profile.append(['nms', (time.time() - t0) * 1000])

        t0 = time.time()
        detections = rescale_boxes(detections[0], img_size, image.shape[:2])
        profile.append(['rescale_boxes', (time.time() - t0) * 1000])

    return profile, detections.numpy()
